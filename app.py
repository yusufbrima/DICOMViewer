...
if uploaded_files:
    ...
    selected_metadata = {
        ...
        "Study Time": dicom_data.get("StudyTime", "N/A"),
        "Study Description": dicom_data.get("StudyDescription", "N/A"),
        "Accession Number": dicom_data.get("AccessionNumber", "N/A"),
        "Study ID": dicom_data.get("StudyID", "N/A"),
        "Series Description": dicom_data.get("SeriesDescription", "N/A"),
        "Series Number": dicom_data.get("SeriesNumber", "N/A"),
...
with col1:
    ...
    st.write("### Selected Image Slice")

    if selected_image is not None:
        # Image contrast adjustment sliders
        min_val = st.slider("Min Value", -255, 255, 0)
        max_val = st.slider("Max Value", 0, 255, 255)

        if selected_file:
            windowed_image = normalize_image(dicom_data.pixel_array, min_val, max_val)
        ...
        st.image(DEFAULT_IMAGE_URL, caption="Default Placeholder Image", use_container_width=True)
...
with col2:
    ...
    download_format = st.selectbox(
        "Select format to download", ("PNG", "JPEG", "BMP", "TIFF")
    )
        ...
        if selected_image is not None:
            ...
            if download_format == "PNG":
                ...
                st.download_button(
                    label="Download BMP",
                    data=img_bytes,
                    file_name=f"{filename}.bmp",
                    mime="image/bmp"
                )
            elif download_format == "TIFF":
                img_bytes = BytesIO()
                pil_image.save(img_bytes, format="TIFF")
                img_bytes.seek(0)
                st.download_button(
                    label="Download TIFF",
                    data=img_bytes,
